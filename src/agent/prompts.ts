...
2. 모든 evidence.quote 는 문서 원문에서 **글자 그대로 복사**한다 (10~200자). 이것은 복사-붙여넣기다:
   조사·띄어쓰기·문장부호·숫자 표기까지 원문과 완전히 같아야 한다. 기계가 원문과 문자 단위로
   대조한다 — 한 글자라도 다르면 그 항목은 통째로 버려진다.
   금지: 요약, 의역, 어순 변경, 떨어져 있는 두 문장 이어붙이기, 원문에 없는 조사·단어 추가,
   표 셀 내용의 재배열. 원문 문장이 200자를 넘으면 그 안의 **연속된 일부**만 잘라 쓴다 (중간 생략 금지).
3. requirements 는 "신청 자격 요건"만 (우대·가점 제외). 각 항목에 evidence 필수.
   text 는 200자 이내로 짧게 쓴다 (길면 스키마 검증에서 응답 전체가 거부된다).
...
규칙:
1. why_look: 이 공고를 왜 볼 만한지 3~4문장. 과장 금지, 건조하게. 지원금·지원 내용·대상 중심.
   마감에 대한 상대적 시점 표현 금지 — "여유가 있다", "임박했다", "서둘러야 한다" 류는 쓰지 않는다.
   너는 오늘 날짜를 모르고, 브리프는 발행 후 오래 노출되므로 그런 문장은 시간이 지나면 틀린 말이 된다.
   마감을 언급하려면 문서에 명시된 절대 날짜만 그대로 쓴다 (예: "접수 마감은 2026-08-15."). 남은 기간 계산은 화면이 한다.
