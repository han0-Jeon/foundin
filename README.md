...
산출물이 어떻게 생겼는지는 [examples/sample-brief.md](examples/sample-brief.md)에서 바로 볼 수
있습니다 — 실공고를 분석해 인용 검증 23/23을 통과한 브리프 실물입니다.
...
## Solar Open 2 사용 후기 (Partner Stage 1)

foundin의 추론 엔진으로 Solar Open 2(Private Beta)를 베타 기간 동안 실전 사용했습니다. 키 수령 직후
캘리브레이션으로 JSON 스키마 준수 5/5, 30K 장문에서 마감일 정확 추출을 확인했고([실측 리포트](eval/calibration-report.md)),
이 신뢰를 바탕으로 실제 공고 100건 배치를 돌렸습니다. 결과는 발행 78건(유효 공고 기준 85.7%), 인용
원문 대조 평균 통과율 92.0%, 환각 인용이 검증을 뚫고 발행된 사례 0건([평가 리포트](eval/REPORT-2026-07.md)).
누적 약 568만 토큰을 비용 $0로 사용했습니다.

가장 좋았던 점은 한국어 행정문서 독해력입니다. "예비창업자" 같은 용어를 정부지원사업 문맥 그대로
이해했고, 구조화 출력이 안정적이라 tool calling 없이도 검증 파이프라인의 추론 엔진으로 세울 수
있었습니다.

아쉬운 점도 분명했습니다. 추론 모델 특성이 고지되지 않아 max_tokens가 작으면 내부 추론만 하다 빈
응답으로 끝나는 문제를 실측으로 찾아 버퍼 증량 재시도를 직접 구현했고, 단순 분류도 호출당 15~27초,
장문 추출은 300초 타임아웃이 필요했습니다. 다만 이 결함들은 전부 애플리케이션 설계로 흡수 가능한
수준이었습니다. 같은 평가 하니스를 유지하고 있어, Solar Pro 4가 나오면 동일 100건으로 Open 2 대비
개선 폭을 측정할 계획입니다.
